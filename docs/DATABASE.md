...
| `campaign_drafts`  | One editable draft per contact (`contact_id` unique, cascade): `subject`, `body` (MO-XXXX placeholder), `discount_percent`, projected `discount_expires_at`, `discount_scope` (all \| recommendations \| set, 0058), compact `purchase_summary` (jsonb), `recommended_product_ids`, `low_confidence` |

...
Gib das Ergebnis NICHT roh aus — nutze es um dann show_product oder compare_products zu rufen.`,
  ...
  showProductDesc:
    "Zeigt die Produktkarte für ein Produkt, das du dem Kunden EMPFIEHLST. Die im Chat sichtbaren Produktkarten ergeben sich AUSSCHLIESSLICH aus deinen show_product-Aufrufen — nicht aus der vorretrieveten Vorauswahl. Rufe es genau EINMAL pro Produkt auf, das du in deinem Text tatsächlich empfiehlst, in der REIHENFOLGE deiner Empfehlung. productId MUSS exakt die Katalog-ID des im Text genannten Produkts sein (nicht die eines anderen Treffers). NICHT aufrufen für Produkte, die du nur nebenbei erwähnst, gegenüberstellst oder am Ende verwirfst — und niemals einfach die vorretrieveten Kandidaten durchkarten. Empfiehl nur real existierende, VERFÜGBARE Produkte; für ein Produkt außerhalb der Vorauswahl hol dir zuerst per search_products die echte ID (niemals eine ID raten oder aus dem Gedächtnis nehmen). VARIANTEN: Bei Produkten mit mehreren Varianten (Stärke, Gewicht, Farbe …) kannst du eine KONKRETE Variante ansprechen, indem du die Varianten-ID mit ~ an die Produkt-ID anhängst: `produkt-id~VariantenID` (die IDs stehen im Produktblock in der Varianten-Zeile, z.B. ~40123). Ohne Suffix gilt die Standardvariante. Nutze das Suffix nur, wenn der Kunde die Variante genannt hat oder sein Bedarf sie klar bestimmt — sonst empfiehl das Produkt und frag nach der gewünschten Variante. Niemals eine Varianten-ID raten.",
